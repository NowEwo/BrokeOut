...
class HintElement(Entity):
    ...
    def draw(self) -> None:
        if self.timer > 0:
            ...
            text_rect.center = (self.scene.surface.get_rect().center[0], self.game.config.graphics.render.height - 139)

            self.scene.font.render_to(self.scene.surface, text_rect, self.hint_text, (self.scene.color[0],self.scene.color[1],self.scene.color[2], self.timer),
                                      size=self.size)
