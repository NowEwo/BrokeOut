...
class Ball(Entity):
    ...
    def set_velocity_by_angle(self, angle: int) -> None:
        ...
        self.velocity = [self.speed * math.cos(math.radians(angle)),
                         -self.speed * math.sin(math.radians(angle))]

    def draw(self) -> None:
        """Draw ball and trail"""
        current_pos = (self.pos[0]-self.game.config.game.ball.radius,
    ...
    def update(self) -> None:  # type: ignore
        ...
        if self.on_player:
            ...
            # Wall collisions
            if (self.pos[0] + self.game.config.game.ball.radius > self.scene.bounds['x_max'] or
                    self.pos[0] - self.game.config.game.ball.radius < self.scene.bounds['x_min']):
                self.velocity[0] *= -1

            # Lose the game (Bottom)
            if self.pos[1] + self.game.config.game.ball.radius > self.scene.bounds['y_max']:
                self.scene.trigger_lose()

            # Top
            if self.pos[1] - self.game.config.game.ball.radius < self.scene.bounds['y_min']:
                self.velocity[1] *= -1
